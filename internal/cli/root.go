...
func addInstallFlags(cmd *cobra.Command, f *rootFlags) {
	...
	cmd.Flags().StringSliceVarP(&f.asset, "asset", "a", nil, "filter assets by matcher; regex prefixes: ~, =~, re:, negative prefixes: ^ or not:, escapes: ~~ and ^^, explicit literal: text: (for example ^musl, not:~.*\\.sbom\\.json$, text:~literal); quote patterns starting with ~ so your shell doesn't expand it to a home directory path")
	cmd.Flags().StringSliceVar(&f.ignore, "ignore", nil, "exclude assets by matcher; regex prefixes: ~, =~, re:, negative prefixes: ^ or not: (inverts ignore), escapes: ~~ and ^^, explicit literal: text:; can be specified multiple times; quote patterns starting with ~ so your shell doesn't expand it to a home directory path")
